...
Use Vitest `it.fails()` function to mark a pending testing.

- If the test fails as expected (because of the `it.fails()`): The test is considered as pending and does not cause a failure of the test suite.
- If the test passes (it "fixes" itself): Vitest will fail the test run to notify you that the reason for pending is gone and the pending tag can be removed.

Tests should be committed in a **pending state** rather than failing. This ensures:
...
```javascript
// vitest
it.fails("should calculate discount for bulk orders", () => {
